...
static void key_press_handle(int param, int repeat_count)
{
	...
	if (is_combination_pressed)
		ssz_assert(i < ssz_array_size(g_combination_key_infos));
	// if it is combination key and it has bind fun, handle the key with null event fun
	if (is_combination_pressed &&
		(g_combination_key_infos[i].handler[kKeyEventPressed] || g_combination_key_infos[i].handler[kKeyEventReleased]))
	{
		if (repeat_count > 0)
		{
			return;
...
static void key_release_handle(int param)
{
	...
	for (i = 0; i < ssz_array_size(g_combination_key_infos); i++)
	{
		...
		if ((param == g_combination_key_infos[i].key1 && key_is_pressed(g_combination_key_infos[i].key2)) || (param == g_combination_key_infos[i].key2 && key_is_pressed(g_combination_key_infos[i].key1)))
		{
