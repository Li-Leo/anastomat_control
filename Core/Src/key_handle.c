...
{
    m2_motor_stop();
    g_m2_motor_run_is_enable = false;
    timer_start_oneshot_after(kTimerDelay2s, MIDDLE_DELAY);
}
...
{
    if (m1m2_switch_is_closed() == false && !m2_motor_is_running() && key_is_pressed(kRLimit) == false && g_m2_motor_run_is_enable == true)
    {
        m2_motor_run(kForward);
        g_r_switch_is_pressed = true;
    }

    if (m1m2_switch_is_closed()) {
        g_r_switch_counter++;
        
        if (g_r_switch_counter == 1) {
            g_pass_commbination[0] = 1;
        } else if (g_r_switch_counter == 2) {
            g_pass_commbination[1] = 1;
        } else {
...
{
    if (m1_motor_is_running() && m1_motor_running_dir() == kForward) {
        m1_motor_stop();
        buzzer(5);
        g_m1_motor_run_backward_is_enable = true;
    }
}
...
void set_key_handler(void)
{
    ...
    key_set_handler(kKeyStopSwitch, kKeyEventReleased, on_released_stop_switch_key);
