...
void on_pressed_return_switch_key(KeyID key, int repeat_count)
{
    ...
    // timer_set_handler(kTimerMotor1Expired, m1_motor_stop_backward);
    // timer_start_oneshot_after(kTimerMotor1Expired, 6950);
}


void on_pressed_stop_switch_key(KeyID key, int repeat_count)
{
    if (m1_motor_running_dir() == kBackward) {

        m1_motor_stop();
        HAL_Delay(500);

        // m2_motor_run(m2_motor_running_dir());
        timer_set_handler(kTimerDelay, start_motor2);
        timer_start_oneshot_after(kTimerDelay, 5000);
    }
}
